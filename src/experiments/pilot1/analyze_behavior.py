...
def main():
    data_dir = 'reformatted_data/'
    filepaths = [filepath for filepath in os.listdir(data_dir) if filepath[0] != '.']
    ...
    for filepath in filepaths:
        ...
        xticks = [df[df['BlockNumber'] == block_num]['BlockType'].iloc[0] + f'\n{block_num+1}' for block_num in range(8)]
        ...
        plt.title(f'Subject {filepath[:-4]}')

        plt.savefig(f'figures/bars/acc/{filepath[:-4]}')
        # plt.show()
        # break
        plt.clf()
    ...
    plt.legend(loc='upper right')
    plt.xticks(list(range(28)), subjects[indices], rotation=45, ha="right")
...
def block_exp_acc(df, block_num):
    block = df[df['IsExp'] & (df['BlockNumber'] == block_num)]['Correct']
    return block.mean()
...
def block_noexp_acc(df, block_num):
    block = df[(1 - df['IsExp']) & (df['BlockNumber'] == block_num)]['Correct']
    return block.mean()
